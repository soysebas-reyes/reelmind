...
Cómo se construye, verifica y publica un instalador de ReelMind. La distribución es
**GitHub Releases** (repo público `soysebas-reyes/reelmind`) con **auto-update** vía
`electron-updater`.
...
| Artefactos | `release/ReelMind-<versión>-setup.exe` + `latest.yml` + `.blockmap` |
...
4. **Construir + publicar** (necesita `GH_TOKEN` con scope `repo`):
   ...
   **Alternativa manual** (sin `GH_TOKEN`): `npm run dist` y después
   ...
   ```bash
   gh release create v0.x.y release/ReelMind-*-setup.exe release/latest.yml release/*.blockmap \
     --title "ReelMind v0.x.y" --notes "…" --draft
   ```
...
- `npm run pack` genera `release/win-unpacked/ReelMind.exe` sin instalador (rápido).
- Instalá el `setup.exe` en una máquina/VM limpia y verificá:
  ...
  - desinstalador presente en "Aplicaciones instaladas" (no borra `%APPDATA%\ReelMind`).
...
- **Certificado OV** (p. ej. Certum "Open Source Code Signing", ~€70/año — aplica
  porque ReelMind es GPL): configurar `CSC_LINK`/`CSC_KEY_PASSWORD` o
